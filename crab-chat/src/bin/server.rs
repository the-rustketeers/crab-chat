    ...
    for stream in listener.incoming() {
        match stream {
            Ok(s) => {
                ...
                // Two declared strings to be used for the reading.
                let mut prev_string: String = "".to_string();

                tpool.execute(move || {
                    loop {
                        std::thread::sleep(time::Duration::from_millis(100));
                        // Loops through indefinitely; an attempt to read from channel across multiple threads.
                        match thread_rx.recv() {
                            // This match case may not be entirely useful yet, I'm not sure.
                            Ok(send_string) => {
                                // Right now the main issue is getting the information from the out-end of the channel
                                if send_string == prev_string {
                                    // to be read from every thread. The information DOES NOT stay there, and I'm done trying to
